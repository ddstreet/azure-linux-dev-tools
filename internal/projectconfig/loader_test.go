...
func TestLoadAndResolveProjectConfig_MergesComponents(t *testing.T) {
	testFiles := []struct {
		path     string
		contents string
	}{
...
`},
		{"/project/include.toml", `# Generated by azldev component update-upstream-commit.
[components.abc]
spec = { type = "upstream", upstream-commit = "abcdef1234567" }

[components.orphan]
spec = { type = "upstream", upstream-commit = "1234567abcdef" }
`},
		{"/project/later.toml", `
...
`},
	...
	assert.NotContains(t, config.Components, "orphan")
}

func TestLoadAndResolveProjectConfig_LegacyGeneratedCommitMarker(t *testing.T) {
	const (
		rootConfig = `
includes = ["include.toml"]
`
		generatedConfig = `# Generated by azldev component update.
[components.orphan.spec]
type = "upstream"
...
`
	...
	require.NoError(t, fileutils.WriteFile(
		ctx.FS(), testConfigPath, []byte(rootConfig), fileperms.PrivateFile,
	))
	require.NoError(t, fileutils.WriteFile(
		ctx.FS(), "/project/include.toml", []byte(generatedConfig), fileperms.PrivateFile,
	))
...
func TestLoadAndResolveProjectConfig_MergesComponentsAcrossFiles(t *testing.T) {
	const configContents1 = `
[components.foo]
`
...
`
	...
	config, err := loadAndResolveProjectConfig(ctx.FS(), false, configPath1, configPath2)
	require.NoError(t, err)
	require.Contains(t, config.Components, "foo")
}
