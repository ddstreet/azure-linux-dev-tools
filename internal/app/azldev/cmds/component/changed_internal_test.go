...
func TestClassifyAndCompareSources_UnchangedSkipsSourcesComparison(t *testing.T) {
	repo, fromRef, toRef := testRepoWithTwoCommits(t,
		map[string][]byte{
	...
	result, include, err := classifyAndCompareSources(
		...
		locks,
		locks,
		&changedContext{repoRoot: "/", renderedSpecsDir: "/SPECS"},
		fromTree,
		toTree,
		classifyOpts{includeUnchanged: true},
	)
	require.NoError(t, err)
	assert.True(t, include)
	assert.Equal(t, changeTypeUnchanged, result.ChangeType)
	assert.False(t, result.SourcesChange)
}
