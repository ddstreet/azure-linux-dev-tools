...
// Save writes a normal component TOML definition containing upstreamCommit.
func (s *Store) Save(componentName, upstreamCommit string) error {
	...
	config := projectconfig.ConfigFile{}

	exists, err := fileutils.Exists(s.fs, path)
	if err != nil {
		return fmt.Errorf("checking upstream commit TOML %#q:\n%w", path, err)
	}

	if exists {
		data, readErr := fileutils.ReadFile(s.fs, path)
		if readErr != nil {
			return fmt.Errorf("reading upstream commit TOML %#q:\n%w", path, readErr)
		}

		if unmarshalErr := toml.Unmarshal(data, &config); unmarshalErr != nil {
			return fmt.Errorf("parsing upstream commit TOML %#q:\n%w", path, unmarshalErr)
		}
	}

	if config.Components == nil {
		config.Components = make(map[string]projectconfig.ComponentConfig)
	}

	component := config.Components[componentName]
	component.Spec.SourceType = projectconfig.SpecSourceTypeUpstream
	component.Spec.UpstreamCommit = upstreamCommit
	...
	// Keep the generated document valid, ordinary TOML while identifying it as
	// an override layer to the project loader. Without this comment marker, an
	// orphan pin file could be mistaken for a complete component definition.
	data = append([]byte(projectconfig.GeneratedUpstreamCommitMarker+"\n"), data...)
